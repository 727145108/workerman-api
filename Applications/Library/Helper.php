...
namespace Crababy\Library;

use Crababy\Library\LoggerClient;
use Exception;
...
class Helper
{
	...
	public static function logger($tag = 'TAG:', $message = '', $level = Helper::INFO) {
		...
		if(isset(self::$options['recordType']) && self::$options['recordType'] == 'tcp') {
			...
			$logPath = isset(self::$options['logPath']) ? self::$options['logPath'] : ROOT_PATH . '/logs/services/';
			$message = false === $message ? "false" : $message;
			$startLine = self::getClientIp() .  "[" . $level . "] pid[" . getmypid() . "][" . date("m-d H:i:s ") . strtok(microtime(), " ") . "]";;
   ...
   * @return [type]         [description]
   */
  public static function encodeUserToken($userId, $randStr = 'Token') {
    $token = self::to62($userId) . '|' . md5(self::generateRand($randStr));
